...
public class RecyclerAdapter2 extends RecyclerView.Adapter<RecyclerAdapter2.MyHoder>{
    ...
    public RecyclerAdapter2(List<User> list, Context context,String user) {
        this.list = list;
    ...
    public void onBindViewHolder(MyHoder holder, int position) {
        ...
        holder.req.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                myRef.child(collid).child("friends").push().setValue(mylist.getCollid()).addOnCompleteListener(new OnCompleteListener<Void>() {
                    @Override
                    public void onComplete(@NonNull Task<Void> task) {
                        if (task.isComplete()){
                            myRef.child(collid).child("requests").setValue(null);
                            myRef.child(mylist.getCollid()).child("requested").setValue(null);
                            myRef.child(mylist.getCollid()).child("friends").push().setValue(collid).addOnCompleteListener(new OnCompleteListener<Void>() {
                                @Override
                                public void onComplete(@NonNull Task<Void> task) {
